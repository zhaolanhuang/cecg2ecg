...
ex = Experiment(f'Reconstruction-MultiCNN-{now_str}')
...
def log_performance(_run, logs):
    ...
    _run.log_scalar("training.loss.recg", float(logs.get('recg')))
    ...
    _run.log_scalar("validation.loss.recg", float(logs.get('val_recg')))
...
def define_and_train(_run, network_structure, validation_split,
    ...
    ckpt_path = f'model_MultiCNN_test_patient{patient_for_test}_{now_str}.h5'
    ...
    for _layer_cfg in network_structure:
        ...
        x_a = layer(**args)(x_a)
        x_theta = layer(**args)(x_theta)
        x_c = layer(**args)(x_c)
        
    y_a = Dense(6, activation = 'linear', name='a')(x_a)
    y_theta = Dense(6, activation = 'linear', name='theta')(x_theta)
    y_c = Dense(6, activation = 'linear', name='c')(x_c)
    y_params = concatenate([y_a, y_theta, y_c], name='params')
    y_recg = GaussLayer()(y_params)
    model = Model(inputs=[inputs, x_recg], outputs=[y_a, y_theta, y_c])
    model.add_metric(tf.keras.metrics.mean_squared_error(x_recg, y_recg), name='recg')
    ...
    model.fit(x=[x_train, y_train['recg']], y=[ y_train['a'], y_train['theta'], y_train['c'] ],
    ...
    _run.log_scalar("test.loss.recg", float(test_loss[4]))
