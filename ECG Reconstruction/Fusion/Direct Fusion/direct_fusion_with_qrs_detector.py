...
ex = Experiment(f'Direct-Fusion-with-QRS-Dect')
...
def common_config():
    ...
    qrs_detect_result_dir = f'../QRS Detection/CNN_Fusion/unovis/result'
...
@ex.main
def test_and_save(_run, unovis_path, cycles_data_path, lstm_data_path, qrs_detect_result_dir,
            lstm_model_dir, para_est_model_dir, patient_for_test, is_lstm_normalization):
    save_dir = f'./Direct-Fusion-with-QRS-Dect_{now_str}'
    import os
    ...
    for _rec in record_no:
        ...
        # Use detected r loc instead
        resampled_r_loc = np.load(f'{qrs_detect_result_dir}/detected_qrs_rec{_rec}_ep8_th0.7.npy').astype(int)
