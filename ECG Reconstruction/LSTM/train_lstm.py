...
ex = Experiment(f'Reconstruction-LSTM')
...
def common_config():
    ...
    network_structure = [
        ...
        {'layer': 'lstm', 'units': hidden_states, 'return_sequences': True,},
        {'layer': 'lstm', 'units': hidden_states, 'return_sequences': True,},
        {'layer': 'dense', 'units': neuron_fcn, 'activation':'relu'},
        {'layer': 'bn'},
...
def load_data_and_normalization(data_path, patient_for_test, isNormalization):
    """
    Load training data from MAT-File generated by generate_cecg_segments_for_LSTM.m
    """
    ...
    # Normalization: (Channelx - mean(Channelx)) / std(Channelx)
    if isNormalization:
...
#     now_str = time.strftime('%Y-%m-%d-%H_%M_%S',time.localtime(time.time()))
    ...
    for _s in nS:
        now_str = time.strftime('%Y-%m-%d-%H_%M_%S',time.localtime(time.time()))
        for i in range(1,7):
            ex.run(config_updates={'patient_for_test': i, 'hidden_states':_s, 
                           'neuron_fcn':128, 'exp_time': now_str})
    nN = [64,256]
    for _n in nN:
        now_str = time.strftime('%Y-%m-%d-%H_%M_%S',time.localtime(time.time()))
        for i in range(1,7):
            ex.run(config_updates={'patient_for_test': i, 'hidden_states':100, 
                           'neuron_fcn':_n, 'exp_time': now_str})
